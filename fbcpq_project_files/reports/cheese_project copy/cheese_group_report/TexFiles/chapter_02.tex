...
The feta cheese analysed in this course was purchased from Ängelmark and produced in Greece. It contained 23\% fat and had a maximum moisture content of 56\%. According to the nutrition label, the cheese provides 278 kcal per 100 g, consisting of 23 g fat, 16 g protein, and 5.5 g salt. 
...
Ethyl acetate is an ester known to contribute pleasant fruity and floral notes to cheese, helping to create a flavour balance by reducing the sharpness originating from free fatty acids. 
...
\subsection{Semi-Quantitative Concentrations}
Although concentrations reported in the literature provide a useful reference for the typical volatile composition of Feta cheese, they do not necessarily reflect the specific product analysed in this exercise. Therefore, semi-quantitative evaluation was performed based on the GC-MS analyses conducted during the practical. Chromatographic data were processed and compiled in an Excel spreadsheet, from which peak areas were extracted and compared. Among the previously identified abundant aroma compounds, only 3-methyl-1-butanol ranked among the four most intense volatiles in the analysed Feta sample. Based on the measured peak areas and the internal standard response, semi-quantitative concentrations were calculated according to Equation \ref{eq:semi_quant}.

\begin{equation}
    C_{i} = 0.5 \times (\frac   {A_{peak\_compound}}
                                {A_{peak\_reference}})
    \label{eq:semi_quant}
\end{equation}

The sensory detection thresholds used for Table \ref{tab:semi_quant_feta} were obtained from a reference compilation reporting thresholds defined across multiple studies, serving as a consolidated database for detection limits. The four compounds exhibiting the highest GC-MS peak areas represent the most abundant volatiles in the analysed Feta cheese sample based on semi-quantitative data. The remaining compounds, shown in italics, were included due to their known relevance to cheese aroma as reported by \citeauthor{a01_kondyli2012effect} \cite{a01_kondyli2012effect}.  

\vspace{0.5em}
Among the four most abundant volatiles, only hexanoic acid, ethyl ester exhibited a semi-quantitative concentration (2.775 ppm) below its reported sensory detection threshold (3 ppm). All other compounds among the four most abundant volatiles had concentrations above their respective thresholds, indicating a high detection potential in the analysed cheese.


\begin{table}[h]
\centering
\caption{Semi-quantitative concentrations of selected aroma compounds in the analysed Feta cheese sample, along with their respective sensory detection thresholds and sensoric descriptors, obtained from \citeauthor{b01_thresholds} and \citeauthor{w01_GoodScentsDatabase}, respectively \cite{b01_thresholds, w01_GoodScentsDatabase}.}
\label{tab:semi_quant_feta}
\rowcolors{2}{white}{gray!7}

\begin{tabularx}{\textwidth}{l | r | l | c | r | X}
\makecell[l]{\textbf{Compound}} &
\makecell[r]{\textbf{Peak}\\\textbf{Area}} &
\makecell[l]{\textbf{Chemical}\\\textbf{Group}} &
\makecell[l]{\textbf{Semi-Quantitative}\\\textbf{Concentrations}\\\textbf{[ppm]}} &
\makecell[r]{\textbf{Detection}\\\textbf{Thresholds}} &
\makecell[l]{\textbf{Sensoric}\\\textbf{Descriptors}} \\
\hline
Ethyl acetate                                       & 11,390 & Ester & 4.982 & 0.005 & Etherial, fruity, sweet, grape and rum-like \\

1-Butanol, 3-methyl-                                & 10,948 & Alcohol & 4.790 & 0.004 & Fusel, fermented, fruity, banana, etherial and cognac \\

Hexanoic acid, ethyl ester                          & 6,343 & Ester & 2.775 & 3 & Sweet, fruity, pineapple, waxy, fatty and estry with a green banana nuance \\
Styrene                                             & 6,307 & Benzenoid & 2.759 & 0.08 & Almond \\

\makecell[l]{\textit{Acetic acid, 2-phenylethyl} \\   
\textit{ester}}                                     & \textit{1,371} & \textit{Ester} & \textit{0.600} & \textit{0.019} & \textit{Sweet, honey, floral} \\

\textit{Acetic acid}                               & \textit{1,050} & \textit{Acid} & \textit{0.460} & \textit{22} & \textit{Sharp pungent sour vinegar} \\

\textit{Octanoic acid, ethyl ester}                 & \textit{593} & \textit{Ester} & \textit{0.259} & \textit{3} & \textit{Waxy, sweet, musty, pineapple and fruity with a creamy, dairy nuance} \\

\textit{Decanoic acid, ethyl ester}                 & \textit{230} & \textit{Ester} & \textit{0.101} & \textit{0.023} & \textit{Sweet, waxy, fruity, apple} \\

\textit{1-Butanol }                                 & \textit{191} & \textit{Alcohol} & \textit{0.083} & \textit{0.5} & \textit{Fusel, oil, sweet, balsam, whiskey} \\

\textit{2-Butanol, (R)-}                            & \textit{105} & \textit{Alcohol} & \textit{0.046} & \textit{3.3} & \textit{Oily, wine} \\

\textit{Decanoic acid }                             & \textit{104} & \textit{Acid} & \textit{0.046} & \textit{10} & \textit{Unpleasant, rancid, sour, fatty, citrus} \\
\end{tabularx}
\end{table}

Overall, the semi-quantitative data primarily support relative ranking of aroma compounds and their \textit{potential} sensory significance in the analysed Feta cheese.
